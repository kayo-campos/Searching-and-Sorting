...
def timeToSort(size, case, method):
    if case == "medium":
    ...
    time = timeit.timeit(method + "({})".format(list_to_sort), setup="from __main__ import " + method,
                         number=1)
...
time_to_generate_medium_case = [timeToSort(quantity, "medium", "selectionSort") for quantity in numbers_to_generate]
time_to_generate_worst_case = [timeToSort(quantity, "worst", "selectionSort") for quantity in numbers_to_generate]
